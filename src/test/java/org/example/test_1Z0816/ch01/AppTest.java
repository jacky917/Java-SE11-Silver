...
public class AppTest
     ...
     * 複習：查看 q08
     * 總結：
     * 1.不能在匿名內部類中定義一個建構式。匿名內部類是基於你正在繼承或實作的類別/介面的宣告後的括號內部，所以沒有名稱，因此不能有一個明確的建構式。
     * 2.如果需要給匿名類初始化，可以使用代碼塊的方式。
     * 3.匿名類的代碼塊執行順序在父類的構造器之後。
     * 4.匿名內部類在Java中只能擴展一個類或實現一個介面。你不能同時擴展一個類和實現一個介面，也不能實現多個介面。
     */
