package org.example.test_1Z0815.ch02;
...
public class AppTest
     ...
     * 3.在長整型（L或l）和浮點型（F或f、D或d）中，下劃線不能出現在後綴之前。
     * 4.在二進制（0b或0B）、十六進制（0x或0X）中，下劃線不能出現在前綴和第一個數字之間。
     *   八進制（0）可以！！！
     * 5.byte數據類型和 int short 只是範圍的區別
     * 1.リテラリの先頭と末尾には記述できない
     * 2.記号の前後には記述できない
     * 記号：「.」、「L」、「F」、「0b」、「0x」、八進数は大丈夫です
     * －－－－－－－－－－－－－－－－
     ...
     *   一旦類型被確定，就不能再改變了。例如，你不能將一個數字賦值給 test_q07 中的 value，因為這會在編譯時報錯。
     */
     ...
     * 總結：
     * 1.基本數據類型轉換失敗全都是編譯異常。
     * 2.沒有繼承關係的轉換都是編譯異常，有繼承關係，但強轉失敗則拋出 ClassCastException 異常。
     */
    ...
    public void test_q14() {
        ...
        System.out.println(str1.replaceFirst("aa","b"));

        String str2 = "aaa";
        System.out.println(str2.replace("aa","b"));
    }
     ...
     * 複習：查看以下代碼
     * 總結：
     * 1.三個參數的 replace，替換初始和結束範圍內的字符串
     * 2.StringBuilder的方法調用會改變原對象，而String不會
     */
